...
export const unsaveRecipe = async (user_id, recipe_id) => {
  ...
  if (error) {
      throw new Error(`Error unsaving recipe: ${error.message}`);
  }
};
...
export const fetchUserProfile = async (userId) => {
  ...
  try {
    const { data: profileData, error } = await supabase
      ...
      .select("*")
      .eq("id", userId)
      .single(); // This ensures you get a single object
    ...
    console.log({ profileData });
    return profileData; // Return the profile data if found
  } catch (error) {
