...
struct DiscoverRecipeCardData: Identifiable, Codable, Hashable, Sendable {
    ...
    // Import-source category for the Cookbook Saved filter.
    // Maps the recipe's source/platform to one of the four fixed filter labels.
    var importSourceCategory: String {
        let s = (source ?? "").trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        let rid = id.trimmingCharacters(in: .whitespacesAndNewlines)
        // Ounje-native (generated by Ounje, not imported from an external source)
        if s == "ounje" || s == "concept_prompt" || s == "direct_input" { return "Ounje" }
        // Photo import
        if s == "photo" || s == "media_image" || s == "ounje photo" { return "Photo" }
        // Social media
        if ["tiktok", "instagram", "youtube", "media_video"].contains(s) { return "Social" }
        // Website import (user-imported recipe with a URL, or any other web source)
        if !rid.isEmpty, !s.isEmpty { return "Website" }
        return "Ounje"
    }
