...
const CountCalories = () => {
 ...
 // Show paywall if user is not premium
  ...
  const showActionSheet = () => {
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        ...
        (buttonIndex) => {
          if (buttonIndex === 1) {
            takePhoto(); // Take photo if option 1 is selected
          } else if (buttonIndex === 2) {
            pickImage(); // Pick image if option 2 is selected
          }
        }
      );
    } else {
  ...
  // Function to analyze the uploaded image for calories and macros
  const analyzeImage = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        {
          headers: {
            //change the api api key... date 23/09/2024
            Authorization: "Api-Key 4Go2sMJT.wSih7lvdL0EtskTpf6HEFu5OEPuD4wo5",
      ...
      if (response.status === 200) {
        ...
        const calculateNutrients = (foodItem, multiplier = 1) => {
          ...
          // Recursively calculate for ingredients if present
        ...
        // Extract food items data and calculate nutrients
...
const styles = StyleSheet.create({
  ...
  removeButton: {
    // change the delete icon place.. date 23/09/2024
    position: "absolute",
    top: -1.7,
    right: -1.7,
