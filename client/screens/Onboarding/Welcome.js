...
import Toast from "react-native-toast-message";
import { useAppStore } from "../../stores/app-store"; // Import app store to manage global state

const Welcome = () => {
  // State to control the visibility of Signup and Signin Modals
  const [signupModalVisible, setSignupModalVisible] = useState(false);
  const [signinModalVisible, setSigninModalVisible] = useState(false);

 
  const set_user_id = useAppStore((state) => state.set_user_id); // Access global state to set user ID

  // Handle opening the Signup model
  const handleSignup = () => {
    setSignupModalVisible(true);
  };

  // Handle opening the Signin modal
  const handleSignin = () => {
    setSigninModalVisible(true);
  };

  // Function for guest sign-in, setting a unique guest user ID
  const continueAsGuest = () => {
    const guestUserId = "guest_" + new Date().getTime(); // Generate a unique guest user ID
    set_user_id(guestUserId); // Update the global state with the guest user ID

    // Show toast message to inform the user they're signed in as a guest
    Toast.show({
      type: "success",
      text1: "Guest Sign In",
      text2: "You are now signed in as a guest.",
  ...
  return (
    <ImageBackground source={welcomepic3} style={styles.background}>
      {/* Customizing StatusBar for Android */}
      <StatusBar backgroundColor={"#0001"} />

      {/* Apply LinearGradient to add a dark overlay */}
      <LinearGradient
        colors={["rgba(0,0,0,0.6)", "transparent"]}
        style={styles.overlay}
      />
...
const styles = StyleSheet.create({
  ...
  container: {
    flex: 1,
    justifyContent: "flex-end",
    paddingBottom: "17%", // Adjust padding for button positioning
  },
  headings: {
    marginRight: "12%", // Adjust margins for better alignment
  },
  heading: {
    color: "white",
    fontSize: 24,
  ...
  close: {
    ...
    top: 5,
    right: 15,
    zIndex: 1, // Ensure the close button is above everything else
  },
});
