...
const FirstLogin = ({ onProfileComplete, session }) => {
  ...
  const { loading, setLoading, convertImageToBase64, sendImages } = useImageProcessing();
  ...
  const saveProfile = async (continueWithoutImages = false) => {
    try {
      ...
      if (fridgeImages.length === 1 && !continueWithoutImages) {
        setLoading(false);
        confirmSaveProfileWithoutImages();
        return;
      }

      if (fridgeImages.length > 1) {
  ...
  const questions = [
    <View style={styles.name}>
      ...
      {/*change the UI of the text input... date 23/09/2024 */}
      <TextInput
        style={{
          textAlign: 'center',
          height: 40,
