...
export const MultipleSelectList = ({
  ...
  const [selectedval, setSelectedVal] = React.useState(defaultOptions || []);
  ...
  React.useEffect(() => {
    ...
    if (selectAll && selectedval.length === 0) {
      ...
      setSelected(
        data.map((item) =>
          save === "value" ? item.value ?? item : item.key ?? item.value ?? item
        )
      );
    }
  }, [data]);
  ...
  return (
    <View>
      {dropdown && search ? (
        ...
        >
          <View>
            <Text style={[{ fontWeight: "600" }, labelStyles]}>{label}</Text>
            {showSelectedNumber ? (
              <View
                style={{
                  flexDirection: "row",
