...
const RECIPE_COMPLETION_SONAR_TIMEOUT_MS = Math.max(
  10_000,
  Number.parseInt(process.env.RECIPE_COMPLETION_SONAR_TIMEOUT_MS ?? "60000", 10) || 60_000
);
...
const MAX_SOCIAL_FRAME_COUNT = Math.max(
  12,
  Math.min(Number.parseInt(process.env.RECIPE_INGESTION_MAX_SOCIAL_FRAMES ?? "12", 10) || 12, 16)
);
...
// Without Redis (the managed worker's normal polling mode), acquireRedisLock returns null,
...
async function fetchCompletedRecipeProjectionsByID(recipeIDs) {
  const uniqueIDs = uniqueStrings(recipeIDs.map((value) => normalizeText(value)).filter(Boolean));
  const importedIDs = uniqueIDs.filter((id) => id.startsWith(USER_IMPORTED_RECIPE_TABLE_CONFIG.recipePrefix));
  const publicIDs = uniqueIDs.filter((id) => !id.startsWith(USER_IMPORTED_RECIPE_TABLE_CONFIG.recipePrefix));
  const projectionByID = new Map();
  const importedSelect = "id,title,source,source_platform,recipe_url,original_recipe_url,attached_video_url,hero_image_url,discover_card_image_url,cook_time_text,review_state,created_at,updated_at";
  const publicSelect = "id,title,source,source_platform,recipe_url,original_recipe_url,attached_video_url,hero_image_url,discover_card_image_url,cook_time_text,created_at,updated_at";

  const load = async (table, select, ids) => {
    if (!ids.length) return;
    const rows = await fetchRows(table, select, {
      filters: [`id=in.${buildInClause(ids)}`],
      limit: ids.length,
    }).catch(() => []);
    for (const row of rows) {
      if (row?.id) projectionByID.set(row.id, row);
    }
  };

  await Promise.all([
    load(USER_IMPORTED_RECIPE_TABLE_CONFIG.recipeTable, importedSelect, importedIDs),
    load(PUBLIC_RECIPE_TABLE_CONFIG.recipeTable, publicSelect, publicIDs),
  ]);
...
async function completeJobByCloningGlobalImportedRecipe(job, cachedJob, { workerID, canonicalURL = null, dedupeKey = null } = {}) {
  ...
  const persisted = await persistNormalizedRecipe(cloneSource, {
    ...
    restoreTombstoneAfter: job.created_at ?? job.queued_at ?? nowIso(),
  });
...
async function fetchSavedRecipeTombstone(userID, recipeID) {
  const normalizedUserID = normalizeText(userID);
  const normalizedRecipeID = normalizeText(recipeID);
  if (!normalizedUserID || !normalizedRecipeID) return null;
...
async function removeSavedRecipeTombstone(userID, recipeID) {
  const normalizedUserID = normalizeText(userID);
  const normalizedRecipeID = normalizeText(recipeID);
  if (!normalizedUserID || !normalizedRecipeID) return;
  await deleteRows("saved_recipe_tombstones", [
    `user_id=eq.${encodeURIComponent(normalizedUserID)}`,
    `recipe_id=eq.${encodeURIComponent(normalizedRecipeID)}`,
  ]);
}

async function upsertSavedRecipeForUser(userID, recipeDetail, { restoreTombstoneAfter = null } = {}) {
  if (!userID) return false;
  const savedProjection = buildSavedProjection(recipeDetail);
  if (!savedProjection.recipe_id) return false;
  const tombstone = await fetchSavedRecipeTombstone(userID, savedProjection.recipe_id);
  if (tombstone) {
    const restoreAt = Date.parse(restoreTombstoneAfter ?? "");
    const deletedAt = Date.parse(tombstone.deleted_at ?? "");
    const shouldRestore = Number.isFinite(restoreAt)
      && (!Number.isFinite(deletedAt) || restoreAt > deletedAt);
    if (!shouldRestore) {
    ...
    await removeSavedRecipeTombstone(userID, savedProjection.recipe_id);
  }
...
function calibrateSocialRecipeAssessment(assessment, recipe, source, qualityFlags = []) {
  ...
  return {
    ...
    review_reason: semanticQuality.blockingIssues[0]
      ?? (missingGroundedContextFlag ? "Grounded web context was unavailable, so this complete reconstruction remains a draft." : null)
...
function selectedSocialHeroFrame(source, modelResult = null) {
  ...
  const frames = selectRecipeEvidenceFrameDataURLs(
    source?.frame_data_urls,
    source?.frame_ocr_texts,
...
async function runSocialRecipeCompletionContext(normalizedRecipe, source, { jobID = null, attempt = 1 } = {}) {
      ...
      },
      {
        ...
        content: [
          ...
          JSON.stringify({
            ...
            author_handle: source?.author_handle ?? null,
            description: source?.description ?? source?.meta_description ?? null,
            transcript_text: limitText(source?.transcript_text ?? "", attempt > 1 ? 4_500 : 6_000),
            frame_ocr_text: summarizeFrameOCRTexts(source?.frame_ocr_texts ?? [], {
              maxFrames: attempt > 1 ? 6 : 12,
              textLimit: attempt > 1 ? 400 : 900,
            }),
            ingredient_candidates: source?.ingredient_candidates ?? [],
            instruction_candidates: source?.instruction_candidates ?? [],
          }),
  ...
  try {
    ...
    await recordExternalAICall({
      ...
      outputTokens: responsePayload?.usage?.completion_tokens ?? responsePayload?.usage?.output_tokens ?? null,
      totalTokens: responsePayload?.usage?.total_tokens ?? null,
      metadata: { job_id: jobID, search_query: query, attempt },
      error: response.ok ? null : new Error(responsePayload?.error?.message ?? `Perplexity returned HTTP ${response.status}`),
    }).catch(() => {});
    if (!response.ok) {
    ...
    const retryableStatus = Number(response?.status ?? 0);
    const retryable = [408, 409, 425, 429].includes(retryableStatus)
      || retryableStatus >= 500
      || /timed out|timeout|fetch failed|http 408|http 409|http 425|http 429|http 5\d\d/i.test(errorSummary(error)?.message ?? "");
    if (attempt < 2 && retryable) {
      await delay(750);
      return await runSocialRecipeCompletionContext(normalizedRecipe, source, { jobID, attempt: attempt + 1 });
    }
    if (jobID) {
      await storeArtifact(jobID, {
        artifact_type: "social_completion_context",
        ...
        metadata: { provider: "perplexity", status: "failed", attempts: attempt },
      }).catch(() => {});
    }
    return null;
  }
    ...
    try {
      const hardResponse = await timeRecipeImportStage(
        "web_completion_hard_retry",
        {
          jobID,
          metadata: {
          initial_model: initialCompletionModel,
          ...
          { role: "system", content: RECIPE_FINAL_VALIDATOR_SYSTEM_PROMPT },
          {
            ...
            content: [
              ...
              JSON.stringify({
                ...
                social_completion_context: source.social_completion_context ? {
                  exact_match_supported: source.social_completion_context.exact_match_supported ?? null,
                  match_confidence: source.social_completion_context.match_confidence ?? null,
                  matched_creator_or_source: source.social_completion_context.matched_creator_or_source ?? null,
                  source_supported_ingredients: (source.social_completion_context.source_supported_ingredients ?? []).slice(0, 30),
                  source_supported_steps: (source.social_completion_context.source_supported_steps ?? []).slice(0, 30),
                  completion_ingredients: (source.social_completion_context.completion_ingredients ?? []).slice(0, 40),
                  completion_steps: (source.social_completion_context.completion_steps ?? []).slice(0, 40),
        ...
        if (socialSourceHasPrimaryRecipeEvidence(source)) {
          job = await appendJobEvent(existingJob.id, "not_recipe_gate_overridden", {
            ...
            quality_flags: uniqueStrings([
              ...
              "primary_video_evidence_gate_override",
            ]),
          });
        } else if (socialSourceHasFoodIdentity(source)) {
