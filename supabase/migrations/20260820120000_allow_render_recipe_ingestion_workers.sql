-- Recipe imports now run on a managed Render worker. Keep the VM prefix during
-- cutover so either worker can safely claim work, then retire it in a later migration.
...
AS $$
BEGIN
  IF lower(coalesce(p_worker_id, '')) NOT LIKE 'vm_recipe_ingest%'
    AND lower(coalesce(p_worker_id, '')) NOT LIKE 'render_recipe_ingest%' THEN
    RETURN;
  END IF;
