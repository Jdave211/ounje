-- Make free-trial subscription states explicit in the founder dashboard.
...
SELECT
  ...
  CASE
    WHEN entitlements.source = 'app_store'
      ...
      THEN 'trial_cancelled_access_remaining'
    WHEN entitlements.source = 'app_store'
      AND entitlements.status = 'active'
      AND entitlements.tier <> 'free'
      AND entitlements.expires_at > timezone('utc', now())
    ...
    WHEN entitlements.status IN ('expired', 'revoked', 'inactive')
      AND entitlements.is_on_trial
      THEN 'trial_' || entitlements.status
    WHEN entitlements.status IN ('expired', 'revoked', 'inactive')
  ...
  CASE
    WHEN entitlements.is_on_trial THEN 'free_trial'
    WHEN entitlements.source = 'app_store' AND entitlements.tier <> 'free' THEN 'paid_subscription'
    ELSE 'other'
...
COMMENT ON VIEW public.founder_subscription_dashboard IS
  'Read-only founder dashboard for current App Store subscription state, including free-trial and paid states, joined with profile and latest Apple notification metadata.';
